package com.vasev.trainingapp.core.database.di
 ...
 *
 * Lives in `core:database` so that `app` does not depend on Room directly — it only consumes
 * the DAOs (and, via feature `data` modules, the repositories built on top of them).
 * Живёт в `core:database`, чтобы `app` не зависел от Room напрямую — он только потребляет DAO
 * (и, через `data`-модули фичей, репозитории, построенные на них).
 *
